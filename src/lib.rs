...
fn create_struct(fields: &Vec<Field>, data: Data, generics: &Generics) -> Tokens {
    ...
    let (assigners, optional_assigners, attributes, empty) = create_fields(&fields, nested_names);
...
fn create_fields(
    ...
    let mut optional_assigners = quote!{};
    let mut empty = quote!{};
    for field in fields {
        ...
        if type_name_string.starts_with("Option<") {
            ...
            next_optional_assigner = quote!{ self.#field_name = optional_struct.#field_name; };
            ...
            next_optional_assigner = quote!{ self.#field_name.apply_options(optional_struct.#field_name); };
            ...
            next_optional_assigner = quote!{
                if let Some(attribute) = optional_struct.#field_name {
                    self.#field_name = Some(attribute);
                }
            };
            next_empty = quote!{ #field_name: None, };
        }
        ...
        optional_assigners = quote!{ #optional_assigners #next_optional_assigner };
        attributes = quote!{ #attributes #next_attribute };
        empty = quote!{ #empty #next_empty }
    }

    (assigners, optional_assigners, attributes, empty)
}
